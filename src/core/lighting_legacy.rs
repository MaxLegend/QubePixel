...
///   [40..44]  shadow_params   (vec4: voxel_origin_x, _y, _z, voxel_tex_size)
///             When voxel_origin is None: (bias, shadow_enabled, cascades, map_size)
///   [44..48]  ssao_params     (vec4: enabled, radius, bias, kernel_size)
///   [48..52]  time_params     (vec4: time_of_day, 0, 0, 0)
///   [52..68]  shadow_view_proj (mat4x4) — unused with voxel shadows, kept for layout compat
pub fn pack_lighting_uniforms(
    ...
    // shadow_params: voxel ray-traced shadow parameters
    if let Some(vo) = voxel_origin {
        data[40] = vo.x;
        data[41] = vo.y;
        data[42] = vo.z;
        data[43] = voxel_tex_size;
    } else {
