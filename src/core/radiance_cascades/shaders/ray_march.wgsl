...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
        ...
        if (block_id != 0u) {
            ...
            if (block_opacity > 0.001) {
                ...
                if (tau >= params.opaque_threshold) {
                    // Opaque hit: subtract this step's emission from radiance_in
                    // to avoid double-counting with radiance_out (endpoint emission).
                    // The emission injection pass handles propagation to nearby probes.
                    radiance_in = radiance_in - (prev_t - transmittance) * block_emission;
    ...
    if (!hit_opaque) {
        ...
        if (params.cascade_level == 4u) {
            ...
            // Radiance (energy), NOT display color — must be much dimmer
            // to avoid white overexposure at cascade boundaries.
